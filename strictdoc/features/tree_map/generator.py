...
def is_coverage_normative_node(node: SDocNode, document: SDocDocument) -> bool:
    """
    Whether ``node`` is a requirement-like node in the "Requirements coverage"
    tree maps. Nodes of autogenerated documents (ingested test reports) are
    excluded: their TEST_RESULT nodes are normative but are not requirements.
    """
    if document.autogen:
        return False
    # Keep requirement-bearing sections so their child requirements are not
    # orphaned (and the chart blanked) when the frame is filtered to normative.
    return node.is_normative_node() or (
    ...
    def export(
        ...
        for document_ in traceability_index.document_tree.document_list:
            ...
            for node_, _ in document_iterator.all_content(
                ...
                if node_.is_normative_node() and not document_.autogen:
        ...
        for document_ in traceability_index.document_tree.document_list:
            ...
            for node, context_ in document_iterator.all_content(
                ...
                is_normative = is_coverage_normative_node(node, document_)
